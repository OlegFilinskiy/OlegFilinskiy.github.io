...
$('.lang_desktop_choice').on('click',function() {
	$(this).css('color', '#ff724a');
    $('.lang_desktop').show(200);
});

$('.lang_pad_choice').click(function() {
	$(this).css('color', '#ff724a');
    $('.lang_pad').show(200);
});
...
$(document).mouseup(function (ed){ // событие клика по веб-документу
    var div = $(".popup"); // тут указываем ID элемента
    var popup = $(".popup");
    if (!div.is(ed.target) // если клик был не по нашему блоку
        && div.has(ed.target).length === 0 && !popup.is(ed.target) && popup.has(ed.target).length === 0) { // и не по его дочерним элементам
        popup.hide("slow"); // скрываем его
	    $('.popup__shadow').hide();
    }
});
...
$("#tel").on("countrychange",function(e, countryData) {
    var intlNumber = $("#tel").intlTelInput("getSelectedCountryData");
      var dialCode = '+' + intlNumber['dialCode'];
    $('#tel').attr('value',dialCode);
...
$("#tel_sub").intlTelInput();

$("#tel_sub").on("countrychange",function(e, countryData) {
    var intlNumber = $("#tel_sub").intlTelInput("getSelectedCountryData");
      var dialCode = '+' + intlNumber['dialCode'];
    $('#tel_sub').attr('value',dialCode);
    this.value = this.value;
});
...
$('.interactive_close').on('click', function () {
	$('.interactive').hide('100');
	$('.interactive__shadow').hide();
	// location.reload();
});

$(document).mouseup(function (ed){ // событие клика по веб-документу
    var div = $(".interactive"); // тут указываем ID элемента
    var popup = $(".interactive");
    if (!div.is(ed.target) // если клик был не по нашему блоку
        && div.has(ed.target).length === 0 && !popup.is(ed.target) && popup.has(ed.target).length === 0) { // и не по его дочерним элементам
        popup.hide("slow"); // скрываем его
	    $('.interactive__shadow').hide();
